 ...
 */
/atom/movable/screen/plane_master/proc/relay_render_to_plane(mob/mymob, relay_plane, ourz)
	if(relay && relay?.plane == relay_plane)
		return

	// //if we already have a relay & it's already been linked to this plane, stop
	// if(relay && relay.plane == relay_plane)
	// 	return
	if(!render_target && generate_render_target)
		render_target = "*[name]: AUTOGENERATED RENDER TGT #[(ourz)]"
	...
	//explanation:
	// So basically, the darkness plane sucks. Do not default out multiplicative planes
